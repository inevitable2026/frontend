...
test("위험도는 등급을 지어내지 않고 발생 가능성 × 피해 크기 를 그대로 보인다", () => {
  // 팩트에 매트릭스가 없고 시나리오가 4×3 과 5×4 를 섞어 쓴다. 같은 12 가 행마다
  // 등급이 달라서, 화면이 "높음" 이라고 단정하면 거짓말이 된다.
  assert.equal(
    위험도표시({ 빈도: 3, 강도: 4, 위험도: 12 }),
    "발생 가능성 3 × 피해 크기 4 → 위험도 12",
  );
  assert.equal(위험도표시(undefined), "미기재");
});

test("회사 코드는 이름으로 바뀌고 모르는 코드는 총칭으로 받는다", () => {
  assert.equal(회사표시("sub_seojin"), "서진건설");
  assert.equal(회사표시(undefined), "미지정");
  assert.equal(회사표시("sub_unknown"), "협력사");
});

test("회사표시는 저장소 코드를 화면 문구로 흘리지 않는다", () => {
  // 등록되지 않은 코드를 그대로 돌려주던 자리다. 안전관리자 화면에 `sub_...` 같은
  // 저장소 키가 그대로 찍혔다.
  for (const 미등록 of ["sub_unknown", "co_newco", "낯선값", "sub_seojin_2"]) {
    const 표시 = 회사표시(미등록);
    assert.equal(표시, "협력사");
    assert.ok(!표시.includes(미등록), `${미등록} 가 화면 문구에 그대로 남았다`);
    assert.ok(!/[a-zA-Z_]/.test(표시), `${미등록} 의 표시에 코드 모양 문자가 남았다`);
  }
  // 등록된 코드는 그대로 이름이 나와야 한다 — 전부 "협력사" 로 뭉개면 그것도 거짓이다.
  assert.equal(회사표시("co_hanshin"), "한신종합건설");
});

test("이행확인미비뺀것은 이 화면이 해결한 항목만 뺀다", () => {
  // 시드의 미비는 셋이다. 이 서랍이 해결한 것은 이행확인뿐인데 셋 다 지우면
  // **해결하지 않은 것을 해결했다고 적는 셈**이 된다.
  const 미비 = ["이행확인 9행", "법적 근거란 공란 12행", "개선 후 위험도 미기재 7행"];
  assert.deepEqual(이행확인미비뺀것(미비), [
    "법적 근거란 공란 12행",
    "개선 후 위험도 미기재 7행",
  ]);
});

test("이행확인미비뺀것은 미비가 없어도 견딘다", () => {
  assert.deepEqual(이행확인미비뺀것(undefined), []);
  assert.deepEqual(이행확인미비뺀것([]), []);
});

test("미비가 이행확인뿐이면 다 빠져서 상신 가능이 된다", () => {
  // 이 배열이 비는 것이 곧 "결재 상신을 올릴 수 있다" 의 근거다.
  assert.deepEqual(이행확인미비뺀것(["이행확인 9행"]), []);
});
