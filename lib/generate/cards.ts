...
const SYSTEM = [
  ...
  "- **title 과 summary 는 현장 사람이 읽는 문장입니다. 이 지시문에 나오는 필드 이름을 그 안에 옮겨 적지 마십시오.**",
  "  `blockedByKeys` · `draftForm` · `delegable` · `approval` · `todo` 같은 말은 당신이 값을 채울 칸의 이름이지 안전관리자가 쓰는 말이 아닙니다.",
  "  실제로 그런 카드가 나왔습니다 — 「회의록이 확정되어야 내용이 정해지므로 blockedByKeys로 묶습니다」 가 그대로 보드에 떴습니다.",
  "  뜻을 그대로 사람 말로 쓰십시오: 「회의록이 확정될 때까지는 이 자료를 손댈 수 없습니다」.",
].join("\n");
