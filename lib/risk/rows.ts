...
/**
 * 위험성평가표 서식의 **머리칸**에 들어가는 값. `documentApprovalState` 팩트의
 * `서식` 칸과 같은 이름을 쓴다.
 *
 * **왜 문서 단위인가:** 평가기법(매트릭스)도 결재자도 회의록마다 다르다. 현장 하나에
 * 4x3 회의록과 5x4 회의록이 함께 있는 것이 이 시나리오의 전제다
 * (`lib/detect/rules/t07-score-gap.ts:24`). 현장 단위로 두면 그 차이가 사라진다.
 *
 * **왜 행이 아니라 여기인가:** `riskAssessmentRow` 에는 매트릭스 필드가 아예 없다.
 * 행에서 뽑을 수 있는 것(관리기간·공종분류)은 뽑고, 뽑을 수 없는 것만 여기 둔다.
 *
 * 모든 칸이 선택이다. **모르면 비운다** — 서식은 그리되 채우지 않는다.
 */
export type 문서서식 = {
  /** 서식 왼쪽 위 칸. 예: `건축공사`. */
  업종?: string;
   ...
   * 평가기법. 예: `빈도·강도법 4x3`.
   *
   * 뒤에 붙은 `4x3` 이 **위험도 등급의 유일한 근거**다. 이 값이 없으면
   * `assessment-sheet.ts` 는 위험도 칸에 색을 칠하지 않는다 — 같은 12 가 4x3 에서는
   * 높음이고 5x4 에서는 보통이라, 매트릭스를 모르는 채 칠한 색은 거짓말이 된다.
   */
  평가기법?: string;
  /** 평가의 법적 근거. 예: `산업안전보건법 제36조`. */
  근거?: string;
  장비?: string[];
  자재?: string[];
  /**
   * 서식 오른쪽 위 결재란 세 칸. 값은 **사람 식별자**(`user_park` 등)다.
   * 이름을 모르는 식별자는 `사람표시()` 가 빈칸으로 돌려준다.
   */
  결재자?: { 검토자?: string; 근로자대표?: string; 승인자?: string };
};
 ...
 * 사람 식별자 → 이름.
 *
 * **정본이 둘로 갈려 있다.** `docs/scenario-gimpo-logistics.md:57` 은 `user_choi` 를
 * 「최민서」라고 적고, `components/task-board/presentation.ts:179` 의 `USERS` 는
 * 「최현우」라고 적는다. 여기서는 **코드 쪽(USERS)을 따랐다** — 같은 사람이 카드
 * 담당자 칩에서는 최현우, 내려받은 평가표에서는 최민서로 보이면 그게 더 나쁜 상태다.
 * 어느 쪽이 맞는지 정해지면 **두 자리를 함께** 고쳐야 한다.
 *
 * 하도급사 반장(`sub_seojin_lee` 등)은 시나리오에 역할만 있고 이름이 없어 여기 없다.
 */
export const 사람이름: Record<string, string> = {
  user_park: "박정우",
  user_choi: "최현우",
};

/**
 * 결재란에 찍을 사람 이름. **모르면 빈칸이다.**
 *
 * `회사표시()` 는 모르는 코드를 「협력사」라는 총칭으로 받지만, 사람은 그럴 수 없다.
 * 결재란은 누가 서명했는지를 말하는 칸이라 총칭을 적으면 **없는 서명을 지어내는 셈**이다.
 * 식별자를 그대로 찍는 것도 안 된다 — 서식에 저장소 키가 남는다.
 */
export function 사람표시(code: string | undefined): string {
  if (!code) return "";
  return 사람이름[code] ?? "";
}
