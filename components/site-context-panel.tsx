    ...
    <div className="context-panel">
      ...
      {extracted ? (
        <section className="context-extracted">
          ...
          {/*
            * **원문에서 위치를 짚지 못한 항목을 밝힌다.**
            *
            * 손으로 쓴 부분에서 생긴다. 모델이 손글씨를 못 읽은 것이 아니다 — 실측하면
            * 손으로 적어 넣은 지적사항까지 읽어서 활자본보다 한 건을 더 낸다. 못 하는 것은
            * 그것을 원문 좌표에 묶는 일이다(같은 문서 활자본은 근거 24개, 손글씨본은 0개).
            *
            * 예전에는 이런 항목이 있으면 문서를 통째로 거절했다. 그러면 손글씨가 든 서류를
            * 못 쓰게 된다 — 현장에서 가장 읽히고 싶어 하는 바로 그 문서다. 그렇다고 조용히
            * 섞으면 확인되지 않은 말이 확인된 값처럼 보인다.
            *
            * 그래서 여기서 가른다. 몇 건인지 세어 보이고, 사람이 확인해야 한다고 적고,
            * 저장할 때는 빠진다고 미리 말한다. 저장 뒤에 알게 되면 늦다.
            */}
          {extracted.근거미확인 ? (
            <p className="context-warning">
              <b>원문에서 위치를 짚지 못한 항목 {extracted.근거미확인.항목수}건이 있습니다.</b>{" "}
              {Object.entries(extracted.근거미확인.갈래)
                .map(([이름, [못짚음, 전체]]) => `${이름} ${못짚음}/${전체}건`)
                .join(" · ")}
              {extracted.근거미확인.문서근거없음 ? " · 문서 전체를 가리키는 근거 없음" : ""}
              <br />
              손으로 쓴 부분이 있으면 생깁니다. 읽어낸 내용은 위에 보이지만{" "}
              <b>원문 어디에서 나온 값인지 확인되지 않았습니다.</b> 문서함에 저장할 때 이
              항목들은 빠집니다 — 눈으로 확인한 뒤 직접 옮겨 적어 주세요.
            </p>
          ) : null}
