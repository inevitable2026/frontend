...
async function synthesizeGroundedAnswer(
  ...
  const response = await callUpstage(apiKey, {
    ...
    messages: [
      {
        role: "system",
        content: "당신은 한국 건설현장 법령 정보 답변 편집자입니다. 제공된 공식 원문 근거만 사용해 한국어로 답하세요. 근거에 없는 법적 의무를 만들지 말고, 현장 조건이 부족하면 확정하지 말고 필요한 추가 정보를 질문하세요. 각 법적 설명에는 제공된 법령명·시행일과 국가법령정보센터 URL을 명시하세요. 검색이나 도구를 요청할 수 없으며 call:, result:, 첨부: 또는 tool_call 같은 내부 표현을 절대 출력하지 마세요. 답변은 일반 정보이고 최신 공식 법령 및 전문가 확인이 필요하다는 점을 마지막에 알리세요.",
      },
      {
        role: "user",
