...
export async function POST(req: Request) {
  ...
  if (mode === "live") {
    ...
    if (!(file instanceof File)) {
      return Response.json({ error: "올릴 파일이 없습니다. 문서를 선택한 뒤 다시 올려 주세요." }, { status: 400, headers: HEADERS });
    }
    ...
    if (!Number.isInteger(byteLength) || byteLength < 0 || byteLength > 500_000_000) {
      return Response.json({ error: "데모에 쓸 파일 크기가 올바르지 않습니다. 파일을 다시 선택해 주세요." }, { status: 400, headers: HEADERS });
    }
