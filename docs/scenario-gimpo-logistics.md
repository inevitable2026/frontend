...
## 12. 크레인 배차 변경 이슈 (보드 맨 위 섹션)

2026-08-19 18:22, 대한크레인 배차 메일(`thread_20260819_crane`)이 9월 2일 타설의
25톤 장비를 50톤으로 대체 투입하겠다고 통보합니다. 접지압이 패드당 약 21톤으로
커져 8월 정기 평가서(`ra_2026_08_regular`)의 양중 관련 행이 전제를 잃습니다.

이 장면을 위해 다음을 맞췄습니다.

- 메일 스레드를 이 현장(`gimpo-gochon`)으로 재배정했습니다. 원래 강남 오피스텔에
  붙어 있었는데, 보드의 이슈 섹션이 이 메일을 근거 [1]로 열기 때문입니다.
- `card_reeval_crane_radius` 카드가 50톤 대체 투입 서사를 들고, 회의록 초안 2행
  (RI-10 수정 · RI-13 신규)을 실어 승인 열에 섭니다. 초안이 들어갈 문서가 자신이
  무효화한 문서와 같으므로(`ra_2026_08_regular`) 보드 맨 위의 "위험성평가 이슈"
  섹션(`components/task-board/risk-issue-section.tsx`)이 이 카드를 집어 변경 전/후
  diff 로 펼칩니다.
- 법령 조문 발췌(제38조)와 회사 양식(EQ-07 장비반입검토서)은
  `lib/board/risk-issue.ts` 의 고정 데이터입니다. 커넥터가 없는 상태에서 근거가
  어떻게 보이는지를 보여주기 위한 합성 자료이며, 실제 인용 계약(read_official_law
  원문만 인용)은 그대로 유효합니다.
